...
import com.camera.traccar.DatabaseHelper.DatabaseHandler
import com.camera.traccar.NetworkManager.NetworkHandler
import com.camera.traccar.PositionProvider.PositionListener
import com.camera.traccar.ProtocolFormatter.formatRequest
import com.camera.traccar.RequestManager.RequestHandler
import com.camera.traccar.RequestManager.sendRequestAsync

class TrackingController(private val context: Context) :
        PositionProvider.PositionListener, NetworkManager.NetworkHandler {
    ...
    private val url: String =
            preferences.getString(Constants.KEY_URL, "http://localhost:5055")!!
    ...
    private fun read() {
        log("read", null)
        databaseHelper.selectPositionAsync(
                object : DatabaseHelper.DatabaseHandler<Position?> {
                    override fun onComplete(success: Boolean, result: Position?) {
                        if (success) {
                            if (result != null) {
                                Log.i(TAG, "read result deviceId: " + result.deviceId)
                                if (result.deviceId ==
                                                preferences.getString(Constants.KEY_DEVICE, null)
                                ) {
                                    send(result)
                                } else {
                                    delete(result)
    ...
    private fun send(position: Position) {
        ...
        sendRequestAsync(
                request,
                object : RequestManager.RequestHandler {
